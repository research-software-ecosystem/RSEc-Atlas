<template>
  ...
  <transition name="fade">
    <div v-if="loading || fakeLoading" class="loading-overlay">
      ...
      <p v-if="!fakeLoading" style="font-weight: bold; font-size: 16px">
        Loading bio tools and containers...
      </p>
    </div>
  </transition>
      ...
      >
        <v-card>
          ...
          <v-card-text>
            ...
            {{
              getToolVersion(tool) !== "No Version Info"
...
const filteredItems = computed(() => {
  ...
  filtered = filtered.filter((item) => {
    ...
    if (dataFilter.value !== "All") {
      ...
      if (
        dataFilter.value === "Has Bioconda Package" &&
        !contents.includes("bioconda")
      ) {
        return false;
      }
      if (
        dataFilter.value === "Has Containers" &&
        !contents.includes("biocontainers")
      ) {
        return false;
      }
      if (
        dataFilter.value === "Compatible with Galaxy" &&
...
const pageNumbers = computed(() => {
  ...
  for (
    let i = Math.max(current - 2, 2);
    i < Math.min(current + 3, total);
    i++
  ) {
    pages.push(i);
  }
